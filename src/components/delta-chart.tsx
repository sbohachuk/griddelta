...
/** Загальний графік: країни (spot) або продукти (EU avg Spot/Day/Week/Month + UA) */
export function OverviewChart({ report }: { report: MarketReport }) {
  ...
  const countryData = report.dates.map((date) => {
    ...
    for (const z of ZONES) {
      row[z.id] = convert(report.rows[date]?.[z.id]?.spot ?? null, currency, rate);
    }
  ...
  const domain =
    mode === "countries"
      ? yDomain(countryData, allIds.filter((id) => enabled[id]) as string[])
