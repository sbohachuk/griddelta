...
import {
  fetchCache, sleep, mapPool, lastPoint, lastPointBefore, pointOnDate,
  fetchDayContract, fetchWeekContract, fetchWeekendContract, fetchMonthContract,
  fetchSpot, fetchEurUah, delta, mean, decadeBounds, type EexPoint,
} from "./market-eex";
import {
  loadCachedZoneQuotes, loadCachedEuUa, datesNeedingFetch,
  upsertZoneQuotes, upsertEuUa, fillDeltas,
} from "./market-cache";
...
function emptyCell(): CellQuote {
  return {
    spot: null, dayFutures: null, weekFutures: null, weekendFutures: null, monthFutures: null,
    dayDeltaEur: null, dayDeltaPct: null, weekDeltaEur: null, weekDeltaPct: null,
    weekendDeltaEur: null, weekendDeltaPct: null, monthDeltaEur: null, monthDeltaPct: null,
    dayTradeDate: null, weekTradeDate: null, weekendTradeDate: null, monthTradeDate: null,
  };
}

async function fetchLiveRange(fetchDates: string[], warnings: string[]): Promise<{
  rows: Record<string, Record<ZoneId, CellQuote>>;
  uaUah: Record<string, number>;
  eurUah: number;
}> {
  if (!fetchDates.length) return { rows: {}, uaUah: {}, eurUah: 46.5 };
  ...
  for (let i = 0; i < ZONES.length; i++) {
    monthSeries.push(await fetchMonthContract(ZONES[i], startDate, endDate));
    if (i < ZONES.length - 1) await sleep(350);
  }
  const emptyMonthIdx = monthSeries.map((pts, i) => (pts.length === 0 ? i : -1)).filter((i) => i >= 0);
  ...
  if (emptyDays > 0) {
    warnings.push(`Day: ${emptyDays}/${dayJobs.length} без settlement (forward-fill / кеш заповнить прогалини).`);
  }

  const monthByZone = {} as Record<ZoneId, EexPoint[]>;
  const weekByZone = {} as Record<ZoneId, EexPoint[]>;
  const weekendByZone = {} as Record<ZoneId, EexPoint[]>;
  ZONES.forEach((z, i) => {
    monthByZone[z.id] = monthSeries[i];
    weekByZone[z.id] = weekSeries[i];
    weekendByZone[z.id] = weekendSeries[i];
    if (monthSeries[i].length === 0) {
      warnings.push(`Month ${z.id} (${z.monthCode}): немає settlement у public API — у таблиці буде «—».`);
    }
  });
  ...
  for (const date of fetchDates) {
    ...
    for (const zone of ZONES) {
      ...
      const dayPt = date <= today ? lastPointBefore(dayPts, date) : lastPoint(dayPts) ?? lastPointBefore(dayPts, date);
      const weekPt = pointOnDate(weekByZone[zone.id] ?? [], date) ?? lastPoint(weekByZone[zone.id] ?? []);
      const weekendPt = pointOnDate(weekendByZone[zone.id] ?? [], date) ?? lastPoint(weekendByZone[zone.id] ?? []);
      const monthPts = monthByZone[zone.id] ?? [];
      const monthPt = pointOnDate(monthPts, date) ?? lastPointBefore(monthPts, addDaysIso(date, 1)) ?? lastPoint(monthPts);
      ...
      rows[date][zone.id] = {
        spot: spotPx, dayFutures: dayPx, weekFutures: weekPx, weekendFutures: weekendPx, monthFutures: monthPx,
        dayDeltaEur: dDay.eur, dayDeltaPct: dDay.pct, weekDeltaEur: dWeek.eur, weekDeltaPct: dWeek.pct,
        weekendDeltaEur: dWeekend.eur, weekendDeltaPct: dWeekend.pct, monthDeltaEur: dMonth.eur, monthDeltaPct: dMonth.pct,
        dayTradeDate: dayPt?.tradeDate ?? null, weekTradeDate: weekPt?.tradeDate ?? null,
        weekendTradeDate: weekendPt?.tradeDate ?? null, monthTradeDate: monthPt?.tradeDate ?? null,
      };
    }
  }
...
function applyForwardFills(dates: string[], rows: MarketReport["rows"]) {
  for (const zone of ZONES) {
    ...
    for (const date of dates) {
      ...
      if (!cell) continue;
      if (cell.dayFutures !== null) { last = cell.dayFutures; lastTd = cell.dayTradeDate; }
      else if (last !== null) {
        cell.dayFutures = last; cell.dayTradeDate = lastTd;
        const dDay = delta(last, cell.spot);
        cell.dayDeltaEur = dDay.eur; cell.dayDeltaPct = dDay.pct;
      }
    }
  }
  for (const zone of ZONES) {
    ...
    for (const date of dates) {
      const cell = rows[date]?.[zone.id];
      if (!cell) continue;
      if (cell.monthFutures !== null) { last = cell.monthFutures; lastTd = cell.monthTradeDate; }
      else if (last !== null) {
        cell.monthFutures = last; cell.monthTradeDate = lastTd;
        const dM = delta(last, cell.spot);
        cell.monthDeltaEur = dM.eur; cell.monthDeltaPct = dM.pct;
      }
    }
  }
  function isoWeekKey(iso: string): string { return String(isoWeekMaturity(iso)); }
  ...
  // Week/Weekend: EEX public often empty; synthesize from Day archive, else Spot (RO/BG/PL)
  for (const zone of ZONES) {
    for (const [, weekDates] of byWeek) {
      ...
      const weekAvg = dayVals.length ? Math.round((dayVals.reduce((a, b) => a + b, 0) / dayVals.length) * 100) / 100 : null;
      const weAvg = weVals.length ? Math.round((weVals.reduce((a, b) => a + b, 0) / weVals.length) * 100) / 100 : weekAvg;
      for (const d of weekDates) {
        const cell = rows[d]?.[zone.id];
        if (!cell) continue;
        if (cell.weekFutures === null && weekAvg !== null) {
          ...
          cell.weekDeltaEur = dW.eur; cell.weekDeltaPct = dW.pct;
        }
        if (cell.weekendFutures === null && weAvg !== null) {
          cell.weekendFutures = weAvg;
          const dWe = delta(weAvg, cell.spot);
          cell.weekendDeltaEur = dWe.eur; cell.weekendDeltaPct = dWe.pct;
        }
      }
    }
  }
...
export const loadMarket = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MarketReport> => {
    ...
    if (cachedCount > 0) {
      warnings.push(`Кеш БД: ${cachedCount}/${dates.length} минулих днів з фактом (spot) — без повторного запиту EEX/Energy-Charts.`);
    }
    if (needFetch.length > 0) {
      warnings.push(`Живе оновлення: ${needFetch[0]}…${needFetch[needFetch.length - 1]} (${needFetch.length} дн.).`);
    } else {
    ...
    if (needFetch.length > 0) {
      ...
      for (const d of dates) {
        ...
        if (e?.eurUah) { eurUah = e.eurUah; break; }
      }
    }
    ...
    for (const date of dates) {
      ...
      for (const zone of ZONES) {
        ...
        if (live) rows[date][zone.id] = fillDeltas({ ...live });
        else if (cached) rows[date][zone.id] = fillDeltas({ ...cached });
        else rows[date][zone.id] = emptyCell();
      }
    }
    ...
    if (stillMissing.length > 0 && stillMissing.length <= 20) {
      warnings.push(`Дозаповнення прогалин: ${stillMissing.length} днів без spot — повторний запит.`);
      await sleep(1_000);
      ...
      for (const d of stillMissing) {
        for (const z of ZONES) {
          ...
          if (g.dayFutures !== null) { cell.dayFutures = g.dayFutures; cell.dayTradeDate = g.dayTradeDate; }
          if (g.weekFutures !== null) cell.weekFutures = g.weekFutures;
          if (g.weekendFutures !== null) cell.weekendFutures = g.weekendFutures;
          if (g.monthFutures !== null) { cell.monthFutures = g.monthFutures; cell.monthTradeDate = g.monthTradeDate; }
    ...
    const euUa: DailyEuUa[] = dates.map((date) => {
      ...
      const uaRdnEur = uaUahPx !== null && eurUah > 0
      ...
      return { date, euAvg, uaRdnEur, uaRdnUah: uaUahPx, deltaPct, eurUah: cached?.eurUah ?? eurUah };
    });
    ...
    const decades: DecadeSummary[] = [...decadeKeys.values()].map((b) => {
      ...
      const disc = (pct: number) => euAvgEur === null ? null : Math.round(euAvgEur * (1 - pct / 100) * 100) / 100;
      const toUah = (eur: number | null) => eur === null ? null : Math.round(eur * eurUah * 100) / 100;
      return {
        label: b.label, periodStart: b.start, periodEnd: b.end,
        euAvgEur, dayAvgEur: dayAvg, weekAvgEur: weekAvg, weekendAvgEur: weekendAvg,
        dayDeltaEur: dDay.eur, dayDeltaPct: dDay.pct, weekDeltaEur: dWeek.eur, weekDeltaPct: dWeek.pct,
        disc30Eur: disc(30), disc20Eur: disc(20), disc10Eur: disc(10),
        disc30Uah: toUah(disc(30)), disc20Uah: toUah(disc(20)), disc10Uah: toUah(disc(10)), eurUah,
      };
    });

    warnings.push("Week/Weekend: де можливо — EEX; інакше середнє Day-архіву або Spot (RO/BG/PL без day-контракту).");

    return {
      startDate, endDate, fetchedAt: new Date().toISOString(), dates,
      zones: ZONES.map((z) => z.id), rows, euUa, decades, warnings,
      sources: {
